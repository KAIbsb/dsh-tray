...
- **生命周期管理**:启动 / 重启 / 停止 / 退出,全部在托盘右键菜单完成;**重启优先走软重启**(从运行中的宿主精确回放启动命令,交给独立助手进程等待端口释放后再拉起),失败自动回退硬重启
- **单击托盘图标**:未运行时自动启动并打开窗口;运行中聚焦已打开的 DSH 窗口(APP 模式或网页标签),没有才开新窗口
...
- **重启零打扰**:重启后已打开的页面走 dsh 官方自动重连自己恢复,托盘不再强制刷新(无重载、无主题闪烁);插件前端更新后按一次 Ctrl+R 即可加载新内容
- **窗口 16:9 自适应**:托盘打开的 APP 模式窗口按主屏工作区 90% 高的 16:9 比例居中打开,不再是浏览器的随机默认尺寸
- **自动认证 token**:打开窗口时解析 harness 日志中的启动地址(dsh ≥ 0.1.2 含 token),并对本机 127.0.0.1 发一次探测;每次打开都会静默续期登录 cookie
- **版本自适应**:按已安装 dsh 的版本自动决定启动参数(兼容 dsh 0.0.1-rc.5 及以上,已对 0.1.5 全线(alpha.1 / alpha.2 / rc.1 / rc.2)完成逐版本触点审计与实测,0.1.6-alpha.1/alpha.2 解包审计触点零变化;0.0.1-rc.1/rc.2 因上游 npm 依赖缺失无法安装,不在支持范围)
...
### 2. 打开方式:APP 模式窗口(默认)或普通网页

harness 的 Web UI 在 `http://127.0.0.1:3080`。托盘「打开窗口」默认用 Chromium 系浏览器的 APP 模式开一个独立窗口(16:9 自适应主屏,不混进标签页);偏好普通标签页的话,在 `dshtray.ini` 设 `openmode=browser`。窗口关了不影响 harness,想再开点一下托盘即可。
...
| 操作 | 行为 |
| --- | --- |
| 左键单击托盘图标 | 未运行:启动;已有 DSH 窗口:聚焦;没有:开新窗口 |
| 右键托盘图标 | 仅弹出菜单 |
...
openmode =                    # 打开方式:app(APP 独立窗口,默认)/ browser(默认浏览器标签页)
```
